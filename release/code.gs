...
var Meta_debuggingMessageFeature = 0;
...
function debug_debugLog() {
  announce_devWarn();
  console.info("System Version: " + Meta_version);
  console.info("Is it debug version? : " + debug_isItDebugVersion());
  console.info("Apps Script: Properties: LINE_ACCESS_TOKEN: " + PropertiesService.getScriptProperties().getProperty("LINE_ACCESS_TOKEN"));
...
function announce_devWarn() {
  console.warn("This code is intended for testing and debugging purposes.");
  console.warn("If you do not intend to use it for testing or debugging, please use the release version.");
}
  ...
  switch (doPost_message) {
    case ("地震情報"):
      ...
      try {
        ...
        throw new Error("Error Details: " + e);
      } finally {
        console.info("Done.");
      }
  }
...
function readEarthquakeDataJson(readEarthquakeDataJson_outPut) {
  ...
  try {
    ...
    console.error("Failed to retrieve the data. Error details: " + e);
    throw new Error("Failed to retrieve the data. Error details: " + e);
  }
  ...
  if (readEarthquakeDataJson_outPut == 1) {
    ...
    if (readEarthquakeDataJson_outPut == 2) {
      console.error("The value of Meta_debuggingMessageFeature is not a specific value.");
      throw new Error("The value of Meta_debuggingMessageFeature is not a specific value.");
    } else {
      console.error("The value of `readEarthquakeDataJson_outPut` is invalid.");
      throw new Error("The value of `readEarthquakeDataJson_outPut` is invalid.");
    }
  }
}
