...
const Meta_version = "1d.2.3";
var Meta_debuggingMessageFeature = 1;
const Meta_updateDate = "2026/07/02";
const Meta_lineMessagingApi = "https://api.line.me/v2/bot/message/reply";
const Meta_isItDebugVersion = debug_isItDebugVersion();

function debug_debugLog() {
  announce_devWarn();
  console.info("[Info]: debug_debugLog: System Version: " + Meta_version);
  console.info("[Info]: debug_debugLog: Is it debug version? : " + debug_isItDebugVersion());
  console.info("[Info]: debug_debugLog: Apps Script: Properties: LINE_ACCESS_TOKEN: " + PropertiesService.getScriptProperties().getProperty("LINE_ACCESS_TOKEN"));
  readEarthquakeDataJson(2);
}

function debug_isItDebugVersion() {
  announce_devWarn();
  if (Meta_debuggingMessageFeature != 0) {
    return "1";
  } else {
    return "0";
  }
...
function announce_devWarn() {
  console.warn("[Warn]: announce_devWarn: This code is intended for testing and debugging purposes.");
  console.warn("[Warn]: announce_devWarn: If you do not intend to use it for testing or debugging, please use the release version.");
}
  ...
  switch (doPost_message) {
    case ("地震情報"):
      ...
      try {
        ...
        throw new Error("[Error]: doPost: Error Details: " + e);
      } finally {
    ...
    case ("debug: testmsg"):
      if (Meta_debuggingMessageFeature == 1) {
        ...
        throw new Error("[Error]: Meta_debuggingMessageFeature: The value of Meta_debuggingMessageFeature is invalid.");
      }
      ...
      if (Meta_debuggingMessageFeature == 1) {
        UrlFetchApp.fetch(Meta_lineMessagingApi, {
          ...
          'payload': JSON.stringify({
            ...
            'messages': [{
              'type': 'text',
              'text': "Debug Info:\nSystem Version: " + Meta_version + "\nIs this a debug version? : " + Meta_isItDebugVersion + "\nDo I like cherry blossoms? : Yes.",
            }],
          }),
        });
        try {
...
function readEarthquakeDataJson(readEarthquakeDataJson_outPut) {
  ...
  try {
    ...
    throw new Error("[Error]: readEarthquakeDataJson: Failed to retrieve the data. Error details: " + e);
  }
  ...
  if (readEarthquakeDataJson_outPut == 1) {
    ...
    if (readEarthquakeDataJson_outPut == 2) {
      ...
      throw new Error("[Error]: readEarthquakeDataJson: The value of `readEarthquakeDataJson_outPut` is invalid.");
    }
  }
}
